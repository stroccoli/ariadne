...
Ariadne's RAG layer retrieves incident pattern documents from a vector store and injects the top-3 results into the analyzer prompt. The knowledge base is populated by the DVC pipeline, which collects data from GitHub issues and public post-mortems, preprocesses, chunks, and indexes them as structured `IngestionDocument` objects.
...
- The ingestion pipeline (DVC) works identically against both endpoints.
...
- Re-indexing requires running the DVC pipeline with `QDRANT_URL` and `QDRANT_API_KEY` set — no background indexing endpoint.
